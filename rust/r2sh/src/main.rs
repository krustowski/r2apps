...
fn show_prompt() {
    ...
    // Fetch system info to render the prompt
    if let Some(info) = abi::sysinfo() {
        user = info.system_user.clone();
        host = info.system_name.clone();
    }
